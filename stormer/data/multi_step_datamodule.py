...
class MultiStepDataRandomizedModule(LightningDataModule):
    ...
    @staticmethod
    def _looks_like_latitudes(coord: np.ndarray, tol: float = 1e-3) -> bool:
        """Return True if `coord` is a plausible vector of geographic latitudes."""
        if coord.ndim != 1:
            return False
        ...
        min_lat = coord.min()
        max_lat = coord.max()
        return (min_lat >= -90.0 - tol) and (max_lat <= 90.0 + tol)

    def get_lat_lon(self):
        # Load coordinate arrays saved by preprocessing.
        ...
        # Some preprocessed 64x32 datasets accidentally store longitude samples
        # inside lat.npy. Detect the issue by checking the coordinate ranges and
        # swap in memory so downstream consumers always receive proper latitudes.
        lat_is_lat = self._looks_like_latitudes(lat)
        lon_is_lat = self._looks_like_latitudes(lon)

        if not lat_is_lat and lon_is_lat:
            rank_zero_warn(
                "Detected latitude values outside [-90, 90] in lat.npy while lon.npy "
                "contains valid latitude samples. Swapping the arrays in-memory so "
                "latitude-dependent metrics stay positive."
            )
            lat, lon = lon, lat
        elif not lat_is_lat and not lon_is_lat:
            raise ValueError(
                "Neither lat.npy nor lon.npy contain plausible latitude samples; "
                "please regenerate the coordinate files."
            )
