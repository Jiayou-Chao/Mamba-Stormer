...
class ERA5MultiStepRandomizedDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        out_transform_mean = torch.from_numpy(self.out_transform_dict[chosen_interval].mean).float()
        out_transform_std = torch.from_numpy(self.out_transform_dict[chosen_interval].std).float()
...
class ERA5MultiLeadtimeDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        inp_data = torch.from_numpy(inp_data).float()
        dict_out = {lead_time: torch.from_numpy(out).float() for lead_time, out in dict_out.items()}
