...
def process_year(year, root_dir, save_dir_split, list_single_vars, list_pressure_vars, list_constant_vars, chunk_size):
    """Process a single year of data."""
    ds_sample = xr.open_dataset(os.path.join(root_dir, list_single_vars[0], f'{year}.nc'))
    if chunk_size is not None:
        n_chunks = len(ds_sample.time) // chunk_size + (1 if len(ds_sample.time) % chunk_size > 0 else 0)
    else:
        n_chunks = 1
    ...
    for chunk_id in range(n_chunks):
        ...
        for i in range(len(list_time_stamps)):
            data_dict = {
...
def create_one_step_dataset(root_dir, save_dir, split, years, list_vars, chunk_size=None, num_workers=None):
    ...
    with ProcessPoolExecutor(max_workers=num_workers) as executor:
        for year in years:
            tasks.append(executor.submit(
                process_year, year, root_dir, save_dir_split, 
                list_single_vars, list_pressure_vars, list_constant_vars, chunk_size
            ))
        ...
        for future in tqdm(as_completed(tasks), total=len(tasks), desc=f'Processing {split}'):
            try:
                ...
                failures += 1
                print(f"Failed processing {split} split task: {exc}")

    if failures:
        raise RuntimeError(f"{split} split failed for {failures} year tasks")
