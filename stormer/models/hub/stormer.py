...
class MemEffAttention(nn.Module):
    ...
    def forward(self, x, attn_bias=None):
        ...
        if memory_efficient_attention is not None:
            try:
                ...
                # Fallback if xformers fails at runtime (e.g. version mismatch)
    ...
    def _fallback_attention(self, q, k, v, attn_bias=None):
        """
        Fallback to PyTorch's native scaled_dot_product_attention (SDPA).
        Shapes:
            q, k, v: (B, N, H, D)
        ...
        # PyTorch SDPA is highly optimized and often as fast as xformers
        x = torch.nn.functional.scaled_dot_product_attention(
            q, k, v, 
            attn_mask=attn_bias,
            dropout_p=0.0 if not self.training else getattr(self, "attn_drop_p", 0.0),
            is_causal=False
        )
...
class Stormer(nn.Module):
    def __init__(self, 
        ...
        # Original size for cropping in forward
        self.original_in_img_size = in_img_size
        
        # Pad both dimensions to be divisible by patch_size
    ...
    def forward(self, x, variables, time_interval):
        ...
        if pad_h > 0 or pad_w > 0:
            # F.pad expects (left, right, top, bottom) for 4D/5D tensors? 
            # Actually for (B, V, H, W) it's (pad_W_left, pad_W_right, pad_H_top, pad_H_bottom)
            # Our H, W are swapped in in_img_size relative to (B, V, H, W) output, 
            # but embedding expects x of shape (B, V, in_img_size[0], in_img_size[1])?
            # Let's check WeatherEmbedding.forward
