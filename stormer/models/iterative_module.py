...
class GlobalForecastIterativeModule(LightningModule):
    ...
    def forward_train(self, x: torch.Tensor, variables, interval_tensors, mean_diff_transform, std_diff_transform):
        ...
        for i in range(n_steps):
            ...
            # Cast to float32 for physical-space accumulation to avoid fp16 precision loss
            # (e.g., MSLP ~101325 Pa overflows fp16; temperature loses ~0.25K precision)
            raw_pred_diff = norm_pred_diff.float() * std_diff_transform.float() + mean_diff_transform.float()
            pred = self.reverse_inp_transform(x.float()) + raw_pred_diff
            x = self.inp_transform(pred).to(dtype=norm_pred_diff.dtype)
        return norm_diffs
    ...
    def forward_validation(self, x: torch.Tensor, variables, interval, steps):
        ...
        for _ in range(steps):
            ...
            # Cast to float32 for physical-space accumulation to avoid fp16 precision loss
            pred_diff_f32 = self.reverse_diff_transform[interval](pred_diff.float())
            pred = self.reverse_inp_transform(x.float()) + pred_diff_f32
            x = self.inp_transform(pred).to(dtype=pred_diff.dtype)
        return x
